<?php
/**
 * LLMa.php - Unified Microframework & Enterprise AI Placeholder
 * (c) 2025 John Mahugu et al. | MIT License
 *
 * This file is based on Jogu.php, now renamed to LLMa.php.
 * Below the microframework classes, you will find a clear placeholder
 * where you can paste your original llama.php enterprise framework code.
 *
 * After pasting, your LLMa.php will become a fullstack LLM+Web microframework!
 */

// -----------------------------------------------------------------------------
// Microframework + SleekDB JSON NoSQL Store (from Jogu.php)
// -----------------------------------------------------------------------------
...
class LLMaMicroApp {
    ...
    protected function detectBaseDir(): string {
        ...
        $base = $documentRoot && $frameworkDir ? str_replace('\\', '/', substr($frameworkDir, strlen($documentRoot))) : '';
        if ($base === false || $base === '') {
            return '/';
        }
        return '/' . trim($base, '/') . '/';
    ...
    protected function createHtaccessIfNeeded() {
        ...
        if (!file_exists($htaccessFile)) {
            $content = <<<HTACCESS
# Auto-generated by LLMa.php PHP Micro Framework
...
HTACCESS;
    ...
    protected function createDefaultViews() {
        ...
        if (!file_exists($homeViewPath)) {
            $homeTemplate = <<<MYPHP
<?php
/** Default Home View for LLMa Framework */
?>
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><title>Welcome to LLMa</title></head>
<body>
<h1>Welcome to the LLMa Fullstack PHP+LLM Framework</h1>
<p>All features of LLM, RAG, vector DB, and micro web are ready!</p>
<nav>
    <ul>
...
// -----------------------------------------------------------------------------
// === PLACEHOLDER FOR ENTERPRISE LLAMA.PHP CODE ===
// -----------------------------------------------------------------------------
//
//    !! IMPORTANT !!
//    Paste the ENTIRE content of your enterprise llama.php source code
//    here (below this comment, above the epilogue).
//
//    This will merge all enterprise LLM, RAG, vector DB, security, tools,
//    chat providers, embeddings, evaluation, and advanced features into
//    your unified LLMa.php microframework.
//
//    <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//    LLAMA.PHP FULL SOURCE FOLLOWS | JohnMahugu@gmail.com
//    >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//
// -----------------------------------------------------------------------------
...
class LlamaConfig {
    private static array $config = [
        ...
        'embedding_models' => [
            ...
            'gemini'     => 'models/embedding-001', // alias for Gemini
        ],
        ...
        'security' => [
            ...
            'allowed_file_types'              => ['txt', 'pdf', 'docx', 'md', 'json', 'csv', 'png', 'jpg', 'jpeg'],
        ...
        // Vector Store
        'vector_store' => [
            'type'       => 'memory', // memory, file, postgresql, redis, elasticsearch, sqlite
            'connection' => [
                'host'     => 'localhost',
                'port'     => 5432,
                'database' => 'llama',
...
//=================================//
// KESH EXTEND ENTERPRISE          //
//================================//
// =============================================================================
// GEMINI AND COHERE PROVIDERS (Chat + Embeddings)
// =============================================================================

class LlamaGeminiChat extends LlamaBaseChatProvider {
    protected function getDefaultModel(): string {
        return LlamaConfig::get('default_models.gemini', 'gemini-pro');
    }
    protected function getApiUrl(): string {
        $model = $this->model;
        $apiKey = LlamaConfig::get('api_keys.google');
        return LlamaConfig::get('api_urls.google', 'https://generativelanguage.googleapis.com/v1') . "/models/{$model}:generateContent?key={$apiKey}";
    }
    protected function getHeaders(): array {
    ...
    protected function buildRequest(string $question, array $options): array {
        $parts = [];
        if ($this->systemMessage) $parts[] = ['text' => $this->systemMessage];
        foreach ($this->messages as $m) $parts[] = ['text' => $m['content']];
        $parts[] = ['text' => $question];
        return [
            'contents' => [
...
class LlamaGeminiEmbedding implements LlamaEmbeddingInterface {
    private string $model;
    private LlamaHttpClient $client;
    private int $dimension;
    public function __construct(string $model = null) {
        $this->model = $model ?? 'models/embedding-001';
        $this->client = new LlamaHttpClient();
        $this->dimension = 768;
    }
    public function embed(string $text): array {
        $apiKey = LlamaConfig::get('api_keys.google');
        $endpoint = LlamaConfig::get('api_urls.google', 'https://generativelanguage.googleapis.com/v1') . "/models/embedding-001:embedContent?key={$apiKey}";
        $request = ['model' => $this->model, 'content' => $text];
        $response = $this->client->post($endpoint, $request);
        if (!isset($response['embedding']['values'])) throw new LlamaException("No embedding returned from Gemini");
...
class LlamaCohereChat extends LlamaBaseChatProvider {
    protected function getDefaultModel(): string {
        return LlamaConfig::get('default_models.cohere', 'command-r-plus');
    }
    protected function getApiUrl(): string {
        return LlamaConfig::get('api_urls.cohere', 'https://api.cohere.ai/v1') . '/chat';
    }
    protected function getHeaders(): array {
        $apiKey = LlamaConfig::get('api_keys.cohere');
        if (!$apiKey) throw new LlamaConfigException('Cohere API key not configured');
        return ['Authorization: Bearer ' . $apiKey, 'Cohere-Version: 2022-12-06'];
    }
    protected function buildRequest(string $question, array $options): array {
        $messages = [];
        if ($this->systemMessage) $messages[] = ['role' => 'SYSTEM', 'content' => $this->systemMessage];
        $messages = array_merge($messages, $this->messages);
        $messages[] = ['role' => 'USER', 'content' => $question];
        return [
            'model' => $this->model,
            'chat_history' => array_map(fn($m) => ['role' => strtoupper($m['role']), 'message' => $m['content']], $messages),
            'message' => $question,
            'stream' => $options['stream'] ?? false,
            'temperature' => $options['temperature'] ?? 0.7,
            'max_tokens' => $options['max_tokens'] ?? 2000,
...
class LlamaCohereEmbedding implements LlamaEmbeddingInterface {
    private string $model;
    private LlamaHttpClient $client;
    private int $dimension;
    public function __construct(string $model = null) {
        $this->model = $model ?? 'embed-multilingual-v3.0';
        $this->client = new LlamaHttpClient();
        $this->dimension = 1024;
    }
    public function embed(string $text): array {
        $request = ['texts' => [$text], 'model' => $this->model];
        $headers = ['Authorization: Bearer ' . LlamaConfig::get('api_keys.cohere')];
        $response = $this->client->post(LlamaConfig::get('api_urls.cohere', 'https://api.cohere.ai/v1') . '/embed', $request, $headers);
        if (!isset($response['embeddings'][0])) throw new LlamaException("No embedding returned from Cohere");
        return $response['embeddings'][0];
    }
    public function embedBatch(array $texts): array {
        $request = ['texts' => $texts, 'model' => $this->model];
        $headers = ['Authorization: Bearer ' . LlamaConfig::get('api_keys.cohere')];
        $response = $this->client->post(LlamaConfig::get('api_urls.cohere', 'https://api.cohere.ai/v1') . '/embed', $request, $headers);
        if (!isset($response['embeddings'][0])) throw new LlamaException("No embedding returned from Cohere");
...
// =============================================================================
// VISION SUPPORT (OpenAI, Gemini)
// =============================================================================

class LlamaVision {
    public static function describeImage(string $imagePath, string $provider = 'openai', array $options = []): string {
        switch ($provider) {
            case 'openai':
                return self::openAIDescribe($imagePath, $options);
            case 'gemini':
                return self::geminiDescribe($imagePath, $options);
            default:
                throw new LlamaException("Vision not supported for provider: $provider");
        }
    }
    private static function openAIDescribe(string $imagePath, array $options): string {
        $apiKey = LlamaConfig::get('api_keys.openai');
        $endpoint = LlamaConfig::get('api_urls.openai') . '/images/generations';
        ...
        $response = $client->post($endpoint, $request, $headers);
        return $response['data'][0]['url'] ?? '[No description available]';
    }
    private static function geminiDescribe(string $imagePath, array $options): string {
        $apiKey = LlamaConfig::get('api_keys.google');
        $endpoint = LlamaConfig::get('api_urls.google', 'https://generativelanguage.googleapis.com/v1') . '/models/gemini-pro-vision:generateContent?key=' . $apiKey;
        $imageData = base64_encode(file_get_contents($imagePath));
        $client = new LlamaHttpClient();
        $request = [
            'contents' => [[
        ...
        return $response['candidates'][0]['content']['parts'][0]['text'] ?? '[No description available]';
    }
}

// =============================================================================
// FACADE UPDATE TO SUPPORT ALL PROVIDERS
// =============================================================================

class LLMa {
    public static function chat(string $provider = 'openai', string $model = null): LlamaChatInterface {
        return match($provider) {
            'openai' => new LlamaOpenAIChat($model),
            'anthropic' => new LlamaAnthropicChat($model),
            'ollama' => new LlamaOllamaChat($model),
            'cohere' => new LlamaCohereChat($model),
            'gemini' => new LlamaGeminiChat($model),
            default => throw new LlamaConfigException("Unknown chat provider: $provider"),
        };
    }
    public static function embedding(string $provider = 'openai', string $model = null): LlamaEmbeddingInterface {
        return match($provider) {
            'openai' => new LlamaOpenAIEmbeddings($model),
            'ollama' => new LlamaOllamaEmbeddings($model),
            'cohere' => new LlamaCohereEmbedding($model),
            'gemini' => new LlamaGeminiEmbedding($model),
            default => throw new LlamaConfigException("Unknown embedding provider: $provider"),
        };
    }
    public static function vision(string $provider = 'openai', array $options = []): LlamaVision {
        return new LlamaVision();
    }
}
// =============================================================================
...
/*
================================================================================
Deployment & Docker Instructions for LLMa.php
================================================================================

# 1. Dockerfile (Place this in your project root as "Dockerfile")
---------------------------------------------------------------
FROM php:8.2-cli
...
# Expose port and run built-in server
EXPOSE 8080
...
# 3. .env Example (Optional, for API keys/config)
---------------------------------------------------------------
# .env file (NOT mandatory, but recommended for secrets)
OPENAI_API_KEY=sk-...
...
# 4. Kubernetes, Heroku, and Other Cloud Deployments
---------------------------------------------------------------
# - For Kubernetes, wrap the container in a Deployment and Service YAML.
# - For Heroku, set "Procfile" as: web: php -S 0.0.0.0:$PORT -t .
# - For AWS ECS/Fargate, push the docker image to ECR and create a service.

# 5. Production Tips
---------------------------------------------------------------
- Use a reverse proxy (Nginx/Apache) for SSL, domain routing, etc.
- For persistent storage (uploads, vector stores), mount host/data volumes.
- For Redis/DB vector stores, set the config in your env or LlamaConfig.
- For log rotation, mount /tmp or set logging path in config.
...
/*
================================================================================
LLMa.php Feature List, README, and Usage Examples
================================================================================

LLMa.php is a single-file, fullstack PHP framework for LLM AI, RAG, Vector DB, and micro web apps.

--------------------------------------------------------------------------------
Features
--------------------------------------------------------------------------------
- Minimal microframework: Routing, views, auto .htaccess, auto folders, SleekDB JSON NoSQL
- Seamless LLM and RAG integration:
    - Multiple chat providers: OpenAI, Anthropic, Ollama, Gemini (Google), Cohere, Mistral
    - Embeddings: OpenAI, Ollama, Gemini, Cohere
    - Vision: OpenAI vision, Gemini vision (image-to-text)
    - Enterprise vector stores: Memory, SQLite, PostgreSQL, Redis, File/JSON, Elasticsearch
    - Document splitting, loading, and metadata
    - Retrieval-Augmented Generation (RAG) pipeline
    - Advanced: Reranking, multi-query, semantic chunking, metadata filtering
- Security: Input validation, prompt injection detection, file type checks
- Performance: Caching (memory, file, Redis), batch processing, rate limiting, retries
- Logging & Monitoring: Rotating logs, metrics, health check endpoint
- Function calling/tools: Automatic tool execution (OpenAI-style)
- Evaluation: String/semantic similarity metrics (ROUGE, BLEU, cosine)
- Docker/Kubernetes/Cloud-ready deployment
- Extensible: Add your own providers, tools, vector backends, or custom routes
...
2. **Default Home Page**
    ...
    Open http://localhost:8000 (shows microframework welcome page).

3. **Basic Usage in any PHP file**

    // Require LLMa.php
    require_once 'LLMa.php';

    // Example: Simple chat with OpenAI
    $chat = new LlamaChat('openai', 'gpt-4-turbo');
...
5. **Enable/Configure Providers**

    - Set your API keys (in .env or via LlamaConfig::set).
    - Supported: OPENAI_API_KEY, ANTHROPIC_API_KEY, GOOGLE_API_KEY, COHERE_API_KEY, MISTRAL_API_KEY.
    - Edit config array in LlamaConfig for defaults, models, or endpoints.
...
- Project: https://github.com/johnmahugu/LLMa
...
echo "\n<!-- \nLLMa.php v3.0.0 | Micro + LLM Placeholder | Dedicated to יהוה (YHWH)\nMIT License | Framework UUID: hkA0wNAoQ0Wv5KVd0L68NQ\nTimestamp: " . time() . "\n-->\n";